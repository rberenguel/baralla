...
const SUIT_TO_NAME = { '♠': 'spades', '♥': 'hearts', '♣': 'clubs', '♦': 'diamonds' };
const NAME_TO_SUIT = { spades: '♠', hearts: '♥', clubs: '♣', diamonds: '♦' };
    ...
    for (const rawLine of lines) {
        ...
        if (line.startsWith('# ')) {
            ...
            if (line.startsWith('- shuffle')) {
                ...
                const rest = line.slice(6).trim();
                const m = rest.match(/^(?:([a-z][a-z0-9-]*)\s+)?\((-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)\)/);
                if (m) {
                    deckOps.push({
                        op: 'move',
                        id: m[1] || null,
...
export function serializeSetup(title, log, multicolor = false) {
    ...
    const deckOps = log.filter(e =>
        e.type === 'shuffle' || e.type === 'flip' || e.type === 'split' ||
        e.type === 'move'   || e.type === 'joker'
    );
    lines.push('', '## Deck');
...
export function applySetup(app, parsed) {
    ...
    for (const op of parsed.deckOps) {
        ...
        switch (op.op) {
            ...
            case 'flip': {
                targetCards.forEach(c => { c.isFaceUp = !c.isFaceUp; c.z = ++app.maxZ; });
                app.applyItemTransforms();
                break;
            }
            case 'joker': {
...
function _shuffleImmediate(app, cards) {
    ...
    cards.forEach(c => { c.z = ++app.maxZ; delete c._rand; });
    app.applyItemTransforms();
}
